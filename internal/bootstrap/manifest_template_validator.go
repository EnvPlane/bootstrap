...
// ComponentImage<service> variables are generated by the control plane for
// component-catalog projects.  The suffix is a normalized service identifier,
// so accepting this narrow family keeps the template language closed while
// allowing each workload to receive its own immutable image.
func allowedTemplateVariable(name string) bool {
	if _, ok := allowedTemplateVariables[name]; ok {
		return true
	}
	if !strings.HasPrefix(name, "ComponentImage") || len(name) == len("ComponentImage") {
		return false
	}
...
func validateTemplateVariables(file string, kind string, yamlBody string) []ManifestTemplateValidationIssue {
	...
	for _, expression := range expressions {
		...
		if len(match) == 2 {
			...
			hasComponentImage = hasComponentImage || strings.HasPrefix(name, "ComponentImage")
			if !allowedTemplateVariable(name) {
	...
	if strings.EqualFold(strings.TrimSpace(kind), "Deployment") && !hasCommitSHA && !hasComponentImage {
		issues = append(issues, ManifestTemplateValidationIssue{
			File:    file,
			Line:    1,
			Code:    "template.required_variable",
